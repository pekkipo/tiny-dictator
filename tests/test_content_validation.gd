...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_laws().size() == 59, "59 laws loaded")
	_check(repo.get_raw_endings().size() == 15, "15 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 188, "188 decisions loaded for ministan")
	...
	_check(repo.get_raw_follow_up_pools().size() == 7, "7 follow-up pools loaded")
