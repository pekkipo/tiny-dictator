...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_laws().size() == 67, "67 laws loaded")
	_check(repo.get_raw_endings().size() == 16, "16 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 207, "207 decisions loaded for ministan")
	...
	_check(repo.get_raw_follow_up_pools().size() == 9, "9 follow-up pools loaded")
