...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_laws().size() == 74, "74 laws loaded")
	_check(repo.get_raw_endings().size() == 23, "23 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 225, "225 decisions loaded for ministan")
	_check(repo.get_raw_ruler_identities().size() == 7, "7 ruler identities loaded")
	_check(repo.get_raw_arcs().size() == 9, "9 arcs loaded")
	_check(repo.get_raw_crises().size() == 7, "7 crises loaded")
	_check(repo.get_raw_follow_up_pools().size() == 10, "10 follow-up pools loaded")
