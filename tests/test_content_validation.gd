...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_laws().size() == 12, "12 laws loaded")
	_check(repo.get_raw_endings().size() == 11, "11 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 74, "74 decisions loaded for ministan")
	_check(repo.get_raw_ruler_identities().size() == 7, "7 ruler identities loaded")
	_check(repo.get_raw_arcs().size() == 6, "6 arcs loaded")
	_check(repo.get_raw_crises().size() == 6, "6 crises loaded")
