...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_laws().size() == 52, "52 laws loaded")
	_check(repo.get_raw_endings().size() == 13, "13 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 169, "169 decisions loaded for ministan")
	...
	_check(repo.get_raw_follow_up_pools().size() == 5, "5 follow-up pools loaded")
