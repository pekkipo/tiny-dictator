...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_advisors().size() == 11, "11 advisors loaded")
	_check(repo.get_raw_laws().size() == 92, "92 laws loaded")
	_check(repo.get_raw_endings().size() == 46, "46 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 298, "298 decisions loaded for ministan")
	...
	_check(repo.get_raw_follow_up_pools().size() == 19, "19 follow-up pools loaded")
	...
	_check(repo.has_arc("profit_corporate_state"), "profit_corporate_state arc present")
	_check(repo.has_arc("robot_government"), "robot_government arc present")
	_check(repo.has_arc("sell_the_moon"), "sell_the_moon arc present")
	_check(repo.has_arc("whiskers_cat_revolution"), "whiskers_cat_revolution arc present")
	_check(repo.has_arc("zero_government_of_forms"), "zero_government_of_forms arc present")
	_check(repo.has_arc("national_festival_economy"), "national_festival_economy arc present")
	_check(repo.has_arc("international_cheese_crisis"), "international_cheese_crisis arc present")
	_check(repo.has_arc("palace_renovation_scandal"), "palace_renovation_scandal arc present")
	_check(repo.has_decision("cat_faction_proposal"), "cat_faction_proposal present")
	_check(repo.has_decision("whiskers_political_demand"), "whiskers_political_demand present")
	...
	_check(repo.has_decision("maybe_useful_trial"), "maybe_useful_trial present")
	_check(repo.has_decision("profit_partnership_brief"), "profit_partnership_brief present")
	_check(repo.has_decision("ai_admin_pilot"), "ai_admin_pilot present")
	_check(repo.has_decision("moon_budget_proposal"), "moon_budget_proposal present")
	_check(repo.has_ending("experimental_republic"), "experimental_republic ending present")
	_check(repo.has_ending("moon_new_owner"), "moon_new_owner ending present")
	_check(repo.has_law("ai_cabinet_pilot"), "ai_cabinet_pilot law present")
	_check(repo.has_law("moon_replacement_research"), "moon_replacement_research law present")
	_check(repo.has_follow_up_pool("corporate_side_effects"), "corporate_side_effects pool present")
	_check(repo.has_follow_up_pool("moon_noise_pool"), "moon_noise_pool present")
	_check(repo.has_crisis("palace_fire"), "palace_fire crisis present")
	_check(repo.has_crisis("military_mutiny"), "military_mutiny crisis present")
	_check(repo.has_decision("national_power_outage_resolution"), "power outage resolution present")
