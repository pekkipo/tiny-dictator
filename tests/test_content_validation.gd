...
func _test_repository_loads() -> void:
	...
	_check(repo.get_raw_advisors().size() == 10, "10 advisors loaded")
	_check(repo.get_raw_laws().size() == 81, "81 laws loaded")
	_check(repo.get_raw_endings().size() == 37, "37 endings loaded")
	_check(repo.get_all_decisions_for_country("ministan").size() == 262, "262 decisions loaded for ministan")
	_check(repo.get_raw_ruler_identities().size() == 7, "7 ruler identities loaded")
	_check(repo.get_raw_arcs().size() == 14, "14 arcs loaded")
	_check(repo.get_raw_crises().size() == 9, "9 crises loaded")
	_check(repo.get_raw_follow_up_pools().size() == 14, "14 follow-up pools loaded")
