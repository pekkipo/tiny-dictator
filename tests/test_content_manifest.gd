...
func _test_expected_counts(manifest: Dictionary) -> void:
	_check(manifest.get("decisions", []).size() == 262, "manifest has 262 decisions")
	_check(manifest.get("catalogs", {}).get("arcs", []).size() == 14, "manifest has 14 arcs")
	_check(manifest.get("catalogs", {}).get("crises", []).size() == 9, "manifest has 9 crises")
	_check(manifest.get("catalogs", {}).get("laws", []).size() == 81, "manifest has 81 laws")
	_check(manifest.get("catalogs", {}).get("endings", []).size() == 37, "manifest has 37 endings")
	_check(manifest.get("catalogs", {}).get("advisors", []).size() == 10, "manifest has 10 advisors")
	...
	_check(approved == 225, "onboarding+standalone+short_chain+major_arc_pack_a+b+c approved count (got %d)" % approved)
	...
	_check(major_arc_approved == 72, "major_arc class has 72 approved Pack A+B+C (got %d)" % major_arc_approved)
