...
## Milestone 2B-13 Pack D final simulation (5000 runs).
## Run: godot --headless --path . -s tests/run_2b13_sim_5k.gd
...
func _initialize() -> void:
	...
	print("=== 2B-13 Final Simulation (5000 runs, Random) ===")
	...
	var starts: Dictionary = sim.get("arc_start_rates", {})
	var completes: Dictionary = sim.get("arc_completion_rates", {})
	for arc_id in pack_d:
		print("Pack D %s start=%.4f complete=%.4f" % [
			arc_id, float(starts.get(arc_id, 0.0)), float(completes.get(arc_id, 0.0))
		])
